...
make install ship Tcl.html readdir.so array.so clock.so file.so interp.so posix.so regexp.so syslog.so readline.so pack.so tclprefix.so sqlite3.so mk.so sdl.so zlib.so
...
# cleanup
rm -rf install *.gcno *.gcda lcov.txt coverage_html lcov_output_with_exclusions.txt lcov_output_without_exclusions.txt mk/
...
cd $OUT_OF_TREE_DIR && $SRC_DIR/configure && make test
